...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if user_id not in data["started_users"]:
            data["started_users"].append(user_id)
...
async def list_groups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Loop through all group IDs to fetch their details
    for group_id in group_ids:
        try:
            # Attempt to fetch chat information for the group
            chat = await context.bot.get_chat(group_id)

            # Create a hyperlink for public groups or display the title
            if chat.title:
                if chat.username:  # For public groups with a username
                    link = f"https://t.me/{chat.username}"
                    valid_groups.append(f"[{chat.title}]({link})")
                else:
                    valid_groups.append(chat.title)
        except Exception as e:
            # Log the error for debugging
            print(f"Failed to fetch details for group {group_id}: {e}")
            continue

    # Prepare and send the response based on valid groups found
    if valid_groups:
        group_names = "\n".join(valid_groups)
        await update.message.reply_text(
            f"The bot is added to the following valid groups:\n{group_names}\n\nTotal number of valid groups: {len(valid_groups)}",
            parse_mode="Markdown"  # Enable Markdown to format hyperlinks
        )
    else:
...
async def count_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if os.path.exists(data_file):
            with open(data_file, 'r') as f:
                data = json.load(f)
                started_users = data.get("started_users", [])
        else:
            started_users = []
        ...
        logger.error(f"Error in /count_users command: {e}", exc_info=True)
        await update.message.reply_text("An error occurred while counting users.")
...
async def handle_new_message(update, context):
    try:
        ...
        if str(chat_id) in auth_data["group_authorized_users"] and user_id in auth_data["group_authorized_users"].get(str(chat_id), []):
            return
        ...
        if group_config.get("auto_delete", False):
            ...
            if text_auto_delete or is_media_message:
                delete_timer_minutes = group_config.get("delete_timer", 10)  # Default to 10 minutes if no timer is set
                delete_timer_seconds = delete_timer_minutes * 60  # Convert minutes to seconds
                asyncio.create_task(delete_message(context, chat_id, message_id, delete_timer_seconds))
...
async def set_timer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get the timer value from the command argument in minutes
    ...
    group_settings[str(chat_id)]['delete_timer'] = delete_time_minutes  # Save in minutes
...
async def show_group_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get the delete time directly in minutes
    delete_time_minutes = group_setting.get('delete_timer', 'Not set')
    ...
    settings_message = (
        f"Group Settings:\n"
        f"Delete time: {delete_time_minutes} minute(s)\n"
